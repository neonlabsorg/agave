...
/// account-view buffer (enough for an `AccountInfo` / `SolAccountInfo`).
/// Inlined here (decision 1): the `solana-program-subaccount` SDK crate is
/// intentionally absent from this port, so the constant is defined directly.

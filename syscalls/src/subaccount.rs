...
use {
    crate::{
        BPF_ALIGN_OF_U128, Error, consume_compute_meter, translate_slice, translate_slice_mut,
        translate_type, translate_type_mut,
    },
    ...
    solana_pubkey::{MAX_SEEDS, PUBKEY_BYTES, Pubkey},
    solana_sbpf::{
...
/// [`load_subaccount_impl`] must fit within that reservation.
const _: () = assert!(
...
/// `sol_load_subaccount_c` must fit within that reservation.
const _: () = assert!(
...
/// (create / load / unload). Subaccount slots rely on direct-mapped
...
fn check_subaccount_syscall_enabled(
    ...
    // v4 rename: fork's `stricter_abi_and_runtime_constraints`.
...
fn find_or_add_subaccount(
    ...
    // deserialization sees real data instead of empty bytes.
...
declare_builtin_function!(
    ...
    fn rust(
        ...
        // subaccount untouched; a writable load touches it once mutated.
        invoke_context
            .transaction_context
            .accounts()
        ...
        // v4 shape: fork's `configure_next_instruction(program, accounts, dedup,
        // data)` maps to `configure_instruction_at_index(instruction_index,
        // program, accounts, dedup, data, caller_index)` — configure the
        // pre-reserved next trace slot and record the current frame as caller.
        let dedup_map = vec![u16::MAX; MAX_ACCOUNTS_PER_TRANSACTION];
        let subaccount_instruction_index = invoke_context
            .transaction_context
            .get_instruction_trace_length();
        let subaccount_caller_index = invoke_context
            .transaction_context
            .get_current_instruction_index()? as u16;
        invoke_context
            .transaction_context
            .configure_instruction_at_index(
                subaccount_instruction_index,
                system_program_index,
                Vec::new(),
                dedup_map,
                std::borrow::Cow::Borrowed(&[]),
                Some(subaccount_caller_index),
            )?;
        ...
        invoke_context.transaction_context.push()?;
        {
            let instruction_context = invoke_context
                ...
                .get_current_instruction_context()?;
            let mut subaccount =
                instruction_context.try_borrow_subaccount_by_tx_index(subaccount_index, true)?;
        ...
        if lamports > 0 {
            ...
            // routes the borrow into the subaccount lane. The system_processor
            ...
            let payer_pubkey =
                *translate_type::<Pubkey>(memory_mapping, payer_pubkey_addr, check_aligned)?;
        ...
        // sentinel (see parasol SDK W8b-sdk).
        invoke_context
            .transaction_context
            .accounts()
...
// F10: helpers for `sol_load_subaccount_{rust,c}` / `sol_unload_subaccount`.
//
...
fn sync_subaccount_slot_after_mutation(
    ...
    // `virtual_address_space_adjustments`) that the program hasn't moved
...
/// Translate the VM seed array into host slices.
...
fn translate_subaccount_seeds(
    ...
    check_aligned: bool,
    invoke_context: &InvokeContext,
    instruction_context: &solana_transaction_context::instruction::InstructionContext,
...
fn load_subaccount_impl(
    ...
    // (SIMD-0449 / access-violation ranges). The main-account serializer sets
    // it to the first byte of the serialized record (the NON_DUP_MARKER byte);
    // for a slot that byte is the header start, so `vm_addr == vm_header_addr`.
...
declare_builtin_function!(
    ...
    fn rust(
        ...
        let data_guard =
            SubaccountDataGuard::detach_if_loaded(invoke_context, memory_mapping, subaccount_index)?;

        let instruction_context = invoke_context
            .transaction_context
            .get_current_instruction_context()?;
        let mut borrowed =
            instruction_context.try_borrow_subaccount_by_tx_index(subaccount_index, is_writable)?;

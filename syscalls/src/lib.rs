...
use {
    ...
    solana_program_runtime::{
        cpi::{
            CallerAccount, CpiError, SolAccountInfo, SolSignerSeedsC, SyscallInvokeSigned,
            TranslatedAccount, cpi_common, translate_account_infos, translate_accounts_c,
            translate_accounts_rust, translate_instruction_c, translate_instruction_rust,
            translate_subaccounts_common, update_caller_account, update_caller_account_region,
        },
    ...
    solana_transaction_context::{
        ...
        instruction_accounts::InstructionAccount, vm_slice::VmSlice,
    },
    std::{
        alloc::Layout,
...
impl SyscallSelfInvokeRust {
    /// Translates a single-subaccount seed list and resolves the resulting
    /// PDA (owned by the currently-executing program). The first seed is
    /// interpreted as a base pubkey that must be present in the current
    /// instruction's account list; its writable bit propagates to the
    /// subaccount so downstream creation can refuse read-only attempts.
    ...
    fn rust(
        ...
        invoke_context
            ...
            .configure_instruction_at_index(
                ...
                vec![InstructionAccount::new_subaccount(
                    subaccount_index,
                    false,
                    true,
                )],
        ...
        invoke_context.transaction_context.push()?;
        {
            let instruction_context = invoke_context
                .transaction_context
                .get_current_instruction_context()?;
            let mut subaccount = instruction_context.try_borrow_subaccount(0)?;
...
declare_builtin_function!(
    /// F10: record the caller-side AccountInfo slice covering subaccounts so
    /// CPI write-back can reconcile VM-side state with host-side storage.
    ...
    fn rust(
        ...
        if subaccounts_info.len() > solana_program_runtime::cpi::MAX_CPI_ACCOUNT_INFOS {
            return Err(InstructionError::MaxAccountsExceeded.into());
        }

        let syscall_context = invoke_context.get_syscall_context_mut()?;
        syscall_context.subaccounts_infos =
            solana_program_runtime::invoke_context::UntypedVmSlice {
...
declare_builtin_function!(
    ...
    fn rust(
        ...
        {
            return Err(Box::new(SyscallError::SubaccountsNotSupported));
        }
        ...
        let (subaccount_pubkey, is_writable) = {
            let instruction_context = invoke_context
                ...
                .get_current_instruction_context()?;
            let program_id = *instruction_context.get_program_key()?;
            SyscallSelfInvokeRust::translate_subaccount_seeds(
                &program_id,
                seeds_addr,
                seeds_len,
                memory_mapping,
...
impl SyscallInvokeSigned for SyscallSelfInvokeRust {
    ...
    fn translate_subaccounts<'a>(
        ...
        // F10: include `sol_load_subaccount` slots in the CPI sync.
        let mut slot_entries =
            solana_program_runtime::cpi::translate_subaccount_slots::<AccountInfo, _>(
                invoke_context,
                memory_mapping,
                check_aligned,
                CallerAccount::from_account_info,
...
impl SyscallInvokeSigned for SyscallSelfInvokeC {
    ...
    fn translate_subaccounts<'a>(
        ...
        // F10: include `sol_load_subaccount` slots in the CPI sync.
        let mut slot_entries =
            solana_program_runtime::cpi::translate_subaccount_slots::<SolAccountInfo, _>(
                invoke_context,
                memory_mapping,
                check_aligned,
                CallerAccount::from_sol_account_info,

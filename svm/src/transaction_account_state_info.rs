...
mod test {
    ...
    fn test_verify_changes() {
        ...
        // Parasol fork (F2/F3): check_rent_state is neutralized — every pre→post
        // transition is accepted, including Uninitialized → RentPaying. The upstream
        // InsufficientFundsForRent assertion is replaced with an Ok-pin so any
        // restoration of rent enforcement is caught.
        assert!(result.is_ok());
    }
}

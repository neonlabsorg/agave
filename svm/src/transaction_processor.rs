use {
    crate::{
        account_loader::{
            ...
            TRANSACTION_ACCOUNT_BASE_SIZE, TransactionCheckResult, TransactionLoadResult,
...
impl<FG: ForkGraph> TransactionBatchProcessor<FG> {
    ...
    pub fn load_and_execute_sanitized_transactions<CB: TransactionProcessingCallback>(
        ...
        for (tx, check_result) in sanitized_txs.iter().zip(check_results) {
                ...
                TransactionLoadResult::Loaded(loaded_transaction) => {
                    ...
                    match (
                        ...
                        (Ok(_), _) => {
                            ...
                            // `update_accounts_for_successful_tx` populates the loader
                            // cache with post-execution state. The lane lives at
                            // `loaded_transaction.accounts[account_keys.len()..]` and is
                            // keyed by the owner-facing pubkey;
                            // `subaccount_storage_address` is applied inside the balance
                            // collector when reading pre-state from the loader cache.
                            // Failed executions roll back subaccount changes, so we skip
                            // the capture there.
            ...
            // subaccounts the program actually touched. For failed/non-executed
            // transactions, subaccount changes are rolled back, so we pass an
            // empty slice.
            // R2.4/F10/PRS-155: also expose the read-only (unchanged)
            // subaccounts — owner keys only, no balances — so the receipt can
            // list them in a separate lane. Rolled back on failed/non-executed
            // txs, so empty there.
    ...
    fn validate_transaction_fee_payer<CB: TransactionProcessingCallback>(
        ...
        // F2 (gasless): if total_fee == 0 and the fee-payer account does not exist on-chain,
        // synthesize a temporary system-owned zero-lamport account so validation continues.
        // Non-zero-fee transactions still require the fee-payer to exist (defence-in-depth).
    ...
    fn execute_loaded_transaction<CB: TransactionProcessingCallback>(
        ...
        let ExecutionRecord {
            ...
            // R2.4/F10/PRS-155: owner-facing keys of subaccounts accessed but
            // left unchanged this tx. Carried onto the execution details as
            // receipt-only metadata (never committed) so the balance collector
            // can list them in the receipt's unchanged lane.
        ...
        ExecutedTransaction {
            execution_details: TransactionExecutionDetails {
                ...
                // R2.4/F10/PRS-155: read-only subaccounts (owner keys only)
                // carried as receipt metadata so the balance collector can list
                // them in the receipt's unchanged lane. Never committed.
                unchanged_subaccount_addresses,
            },
            loaded_transaction,

use std::{collections::{BTreeSet, BinaryHeap, VecDeque}, fmt::Display, panic, str::FromStr, time::{Duration, Instant}};
...
impl Drop for TxState {
    fn drop(&mut self) {
        if let Self::Active{holder, ..} = self {
            holder.unlink();
        }
    }
}
...
fn main() {
    ...
    loop {
        ...
        for (score, shared_key) in new_txs.into_iter() {
            ...
            let view = SanitizedTransactionView::try_new_sanitized(tx.data.inner_data(), true, true)

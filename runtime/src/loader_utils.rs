...
pub fn instructions_to_load_program_of_loader_v4<T: Client>(
    ...
    let program_keypair = program_keypair.unwrap_or_else(|| {
        ...
        instructions.push(system_instruction::create_account(
            ...
            // (programs/loader-v4/src/lib.rs:217..223). Under zero rent (parasol) the
            // minimum is 0, so without `1.max(..)` the program account would be funded
            // with 0 lamports and the next instruction fails with `InsufficientFunds`.
            // Mirrors the same idiom used by the v3 helper (`create_buffer`, line 87).
            1.max(
                bank_client
                    .get_minimum_balance_for_rent_exemption(

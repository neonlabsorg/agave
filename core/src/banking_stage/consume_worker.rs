    ...
    impl ExternalWorker {
        ...
        fn process_message(
            ...
            if !Self::validate_message(message) {
                return self
                    .return_unprocessed_message(message, processed_codes::INVALID)
                    .map(|()| false);
            }
        ...
        fn execute_batch(
            ...
            for attempt in 0..2 {
                let leader_state = {
                    let mut leader_probe = non_blocking_check_slot_state(&self.shared_leader_state, self.ticks_per_slot);
                    ...
                    if leader_probe.matches(message.max_working_slot) {
                        ...
                        loop {
                            ...
                            core::hint::spin_loop();
                            leader_probe = non_blocking_check_slot_state(&self.shared_leader_state, self.ticks_per_slot)
                        }
        ...
        fn check_status_checks<D: TransactionData>(
            ...
            // The fork's Bank::check_transactions does not return the slot a tx was previously
            // included in; the ALREADY_PROCESSED branch therefore leaves `included_slot` at 0
            // (documented as undefined when the slot can't be sourced).
            let status_check_results = working_bank.check_transactions(
                txs,
            ...
            let mut status_check_results_iter = status_check_results.iter();
                ...
                parsing_and_resolve_results.iter().enumerate()
            {
                ...
                let status_check_result = status_check_results_iter
                    .next()
...
fn non_blocking_check_slot_state(shared_leader_state: &SharedLeaderState, ticks_per_slot: u64) -> CurrentSlotState {
...
impl ConsumeWorkerMetrics {
    ...
    fn update_on_execute_and_commit_timings(
        ...
        self.timing_metrics.execute_compute_subaccounts_us.fetch_add(
            execute_timings.details.compute_subaccounts_us.0,
            Ordering::Relaxed,
        );
...
struct ConsumeWorkerTimingMetrics {
    ...
    execute_compute_subaccounts_us: AtomicU64,
...
impl ConsumeWorkerTimingMetrics {
    fn report_and_reset(&self, id: &str) {
        datapoint_info!(
            ...
            ("execute_compute_subaccounts_us", self.execute_compute_subaccounts_us.swap(0, Ordering::Relaxed), i64),
